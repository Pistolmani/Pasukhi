...
public class InboundMessageConsumerTests
{
    ...
    [Fact]
    public async Task No_match_text_persists_inbound_only_and_publishes_no_outbound()
    {
        var businessId = Guid.NewGuid();
        var channelId = Guid.NewGuid();
        var (harness, provider) = await CreateHarnessAsync(nameof(No_match_text_persists_inbound_only_and_publishes_no_outbound));
        await using (provider)
        {
            await SeedChannelAsync(provider, businessId, channelId);
